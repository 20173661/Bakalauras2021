...
public class OrderElement : MonoBehaviour { 
    ...
    public void NewOrderElement(string _workerID, string _id, string _title, string _price, string _pricePer, string _workercity, string _duration, string _durationPer, string _status, string _category,
        ...
        orderCity.text = clientCity;
        orderAdress.text = clientAdress;
        firebaseManager = GameObject.Find("FirebaseManager");

        if (status == "0") orderStatus.text = "Statusas: Laukiama patvirtinimo";
        else if (status == "1") orderStatus.text = "Statusas: Užsakymas patvirtintas";
        else if (status == "2") orderStatus.text = "Statusas: Užsakymas atšauktas";
        else if (status == "3") orderStatus.text = "Statusas: Užsakymas įvykdytas";

        orderInfoPanel = firebaseManager.GetComponent<DatabaseManager>().WorkerOrderInfoPanel;
    }

    public void OpenWorkerOrderInfo() {

        orderInfoPanel.GetComponent<OrderInfo>().NewOrderElement(workerID, id, title, price, pricePer, workerCity, duration, durationPer, status, category, subcategory, about, jobTags, rating, views,
            reviews, clientID, clientNameSurname, clientNumber, clientAdress, clientCity, clientOrderDate, workerNameSurname, workerNumber, orderTime, additionalInfo);
        orderInfoPanel.SetActive(true);
    }

    public void OpenClientOrderInfo() {
        orderInfoPanel.GetComponent<OrderInfo>().NewOrderElement(workerID, id, title, price, pricePer, workerCity, duration, durationPer, status, category, subcategory, about, jobTags, rating, views,
            reviews, clientID, clientNameSurname, clientNumber, clientAdress, clientCity, clientOrderDate, workerNameSurname, workerNumber, orderTime, additionalInfo);
        orderInfoPanel.SetActive(true);
    }
